...
class KNN_dist(nn.Module):
    def __init__(self,k):
        ...
        # self.se = SELayer(channel=768)
        self.k=k
    def forward(self,F,vertices):
        id = knn(self.k, vertices, vertices)
        F = index_points(F,id)          # k个邻居含自己的特征矩阵  torch.Size([20, 12, 4, 768])
        # F = self.se(F)
...
class View_selector(nn.Module):
    def __init__(self, n_views, sampled_view, nclasses,dim=768):
        ...
        self.cls = nn.Sequential(
            nn.Linear(dim*self.s_views, 256*self.s_views),
            nn.LeakyReLU(0.2),
    ...
    def forward(self,F,vertices,k):
        ...
        F1_id_F = F1_id.unsqueeze(-1).repeat(1, 1, 1, self.dim)
...
class LocalGCN(nn.Module):
    def __init__(self,k,n_views,dim=768):
...
class LocalGCN_raw(nn.Module):
    ...
    def forward(self,F,V):
        ...
        F = F.view(-1, self.dim)
        F = self.conv(F)
        F = F.view(-1, self.n_views, self.dim)
        return F

class NonLocalMP(nn.Module):
    def __init__(self,n_view,dim=768):
    ...
    def forward(self, F):
        ...
        M = torch.sum(M,-2)
        F = torch.cat((F, M), 2)
        F = F.view(-1, self.dim * 2)
        F = self.Fusion(F)
        F = F.view(-1, self.n_view, self.dim)
        return F
