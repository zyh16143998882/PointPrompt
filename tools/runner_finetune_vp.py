...
def run_net(args, config, train_writer=None, val_writer=None, view_points=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            if view_points.size(1) == 20:
                label_ = label.unsqueeze(1).repeat(1, 4 * (10 + 5)).view(-1)
            elif view_points.size(1) == 8:
                label_ = label.unsqueeze(1).repeat(1, 4 * (10 + 0)).view(-1)
            if config.model.NAME == "PointCenterGCNTransformer":
                label_ = label.unsqueeze(1).repeat(1, 4 * (10 + 38)).view(-1)
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            ...
            metrics = validate(base_model, test_dataloader, epoch, val_writer, args, config, logger=logger, view_points=view_points)
...
def validate(base_model, test_dataloader, epoch, val_writer, args, config, logger = None, view_points=None):
